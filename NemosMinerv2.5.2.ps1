param(
    ...
    [String]$Wallet = "3FJxLvN1XTjN5zH78YMVDdPSNrTzN44DeB", 
    [Parameter(Mandatory = $false)]
...
try {
    $Version = Invoke-WebRequest "https://raw.githubusercontent.com/ayoungminer/InfoLookup/master/version.json" -UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} | ConvertFrom-Json 
}
catch {  }
...
while ($true) {
    ...
    if ((Get-Date).AddDays(-1).AddMinutes($Donate) -ge $LastDonated -and ($Wallet -eq $WalletBackup -or $UserName -eq $UserNameBackup)) {
        ...
        if ($Donation) {
            ...
            if ($WorkerName) {$WorkerName = "NemosMinerClassic-v2.5.2"}
        }
    }
