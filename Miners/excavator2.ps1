    ...
    try {
        if ((Get-Algorithm $_) -ne "Decred" -and (Get-Algorithm $_) -ne "Sia") {
            ...
            [PSCustomObject]@{
                Type = "NVIDIA"
                Path = $Path
                Arguments = "-p 4068 -c $($Pools.$(Get-Algorithm $_).Name)_$(Get-Algorithm $_)_$($Pools.$(Get-Algorithm $_).User)_$($Threads)_Nvidia.json -na"
            ...
            [PSCustomObject]@{
                ...
                HashRates = [PSCustomObject]@{"$(Get-Algorithm $_)NiceHash" = $Stats."$($Name)_$(Get-Algorithm $_)NiceHash_HashRate".Day}
                API = "NiceHash"
                Port = $Port
                URI = $Uri
				User = $Pools.(Get-Algorithm($_)).User
