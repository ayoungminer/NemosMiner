...
$Commands = [PSCustomObject]@{
    "balloon" = " -d $SelGPUCC --cuda_threads 256 --cuda_blocks 100" #Balloon
}
...
$Commands | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    [PSCustomObject]@{
        ...
        HashRates = [PSCustomObject]@{(Get-Algorithm($_)) = $Stats."$($Name)_$(Get-Algorithm($_))_HashRate".Day * .87} # substract 13% (testing shows much higher console speed than Actual Pool speed, reduceing recorded console speed to same as pool)
        API = "Ccminer"
        Port = 4068
        Wrap = $false
        URI = $Uri
